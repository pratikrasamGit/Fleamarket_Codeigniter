...
class Rudra_category_details_rudra_model extends CI_Model
                
{
	...
	$query = " SELECT TBL.id, TBL.category_name, TBL.dnk_name, TBL.status, TBL.is_deleted" ;
	$query .= "  FROM $table "; 
	$where = " WHERE 1 = 1 ";
	 $where .= " AND ( ";
	$where .=  "  TBL.id LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.category_name LIKE '%".$filter_data['general_search']."%'";
