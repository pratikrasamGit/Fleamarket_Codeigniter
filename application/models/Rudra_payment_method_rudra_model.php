...
class Rudra_payment_method_rudra_model extends CI_Model
                
{
	...
	$query = " SELECT TBL.id, TBL.name, TBL.pic, TBL.status, TBL.is_deleted" ;
	$query .= "  FROM $table "; 
	$where = " WHERE 1 = 1 ";
	 $where .= " AND ( ";
	$where .=  "  TBL.id LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.name LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.pic LIKE '%".$filter_data['general_search']."%'";
