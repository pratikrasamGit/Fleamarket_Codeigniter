...
class Rudra_plan_rudra_model extends CI_Model
                
{
	...
	$query = " SELECT TBL.id, TBL.plan_name, TBL.dnk_name, TBL.price, TBL.discount_price, TBL.plan_description, TBL.dnk_description" ;
	$query .= "  FROM $table "; 
	$where = " WHERE 1 = 1 ";
	 $where .= " AND ( ";
	$where .=  "  TBL.id LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.plan_name LIKE '%".$filter_data['general_search']."%'";
	...
	$where .=  "  OR  TBL.plan_description LIKE '%".$filter_data['general_search']."%'";
