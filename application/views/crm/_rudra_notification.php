                    ...
                    <div class="row">
                        ...
                        <h5><?= $page_header ?></h5>
                            ...
                            <div class="col-sm-7 text-right">
                                <!-- <button onclick="static_form_modal('rudra_notification/post_actions/get_data?id=0','rudra_notification/post_actions/insert_data','md','Update Details')" type="button" class="btn btn-glow-success btn-info" title="" data-toggle="tooltip" data-original-title="Add New Item" aria-describedby="tooltip651610">New</button>&nbsp; -->
                                <a href="rudra_notification/post_actions/export_data" type="button" class="btn btn-glow-danger btn-warning" title="" data-toggle="tooltip" data-original-title="Export CSV" aria-describedby="tooltip651610">CSV</a>
...
<!--- FORM Modal Ends ---->
  <div class="md-modal md-effect-11 dynamic-modal" id="form_modal_rudra_notification">
       <div class="modal-content">
           <div class="modal-header theme-bg2 ">
               <h5 class="modal-title text-white" id="heading_rudra_notification">Loading...</h5>
               <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
           </div>
           <form id="frm_rudra_notification" method="post" enctype="multipart/form-data">
               <div class="modal-body" id="modal_form_data_rudra_notification"> </div>
...
responsive: true,
"ajax": {
	"url": "<?php echo base_url('rudra_notification/list') ?>",
	"dataType": "json",
	"type": "POST",
	"data": {
...
function static_form_modal(data_url, action_url, mtype, heading) {
    $("#form_modal_rudra_notification").modal();
    $("#form_modal_rudra_notification").addClass('md-show');
    $("#heading_rudra_notification").text(heading);
    $("#frm_rudra_notification").attr("action", '<?= base_url() ?>' + action_url);
    $.ajax({
        type: 'POST',
        ...
        success: function(data) {
            ...
            if (response.status) {
                ...
                $("#modal_form_data_rudra_notification").html(jsonData.form_data);
...
function status(id, status) {
    $.ajax({
        type: 'POST',
        url: '<?= base_url() ?>rudra_notification/post_actions/status',
...
function deletedata(id) {
    $.ajax({
        type: 'POST',
        url: '<?= base_url() ?>rudra_notification/post_actions/delete_data',
        data: {id: id},
        success: function(data) {
            response = JSON.parse(data);
            if (response.status) {
...
$("#frm_rudra_notification").submit(function(e) {
            e.preventDefault();
            $(".btn-modal-form").prop('disabled', true);
            $(".btn-modal-form").html('Wait...');
            var formData = new FormData($("#frm_rudra_notification")[0]);
            var action_url = $("#frm_rudra_notification").attr("action");
            $.ajax({
                type: 'POST',
                url: action_url,
                data: formData,
