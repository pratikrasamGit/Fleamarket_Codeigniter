...
error_reporting(0);
class Rudra_notification_crtl extends CI_Controller
{                   
    public function __construct()
    {
        parent::__construct();                
        $this->bdp = $this->db->dbprefix;
        $this->full_table = 'rudra_notification';
        $this->return_data = array('status' => false, 'msg' => 'Error', 'login' => false, 'data' => array());
        //$this->set_data();
        $this->base_id = 0;
        $this->load->model('crudmaster_model','crd');
        $this->load->model('rudra_notification_rudra_model','rudram');
	...
	**************/
    ...
    public function index()
    {
        ...
        $data['page_template'] = '_rudra_notification';
        $data['page_header'] = ' Notification';
        $data['load_type'] = 'all';                
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'fk_user_id', 'message', 'status', 'is_deleted', );
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'rudra_notification/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'rudra_notification/post_actions';
				$action_url = 'rudra_notification/post_actions';
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_notification_form", $data, TRUE);
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_notification').'_'.date('d-m-Y').".csv";
