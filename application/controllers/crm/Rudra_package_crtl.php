...
error_reporting(0);
class Rudra_package_crtl extends CI_Controller
{                   
    public function __construct()
    {
        ...
        $this->load->model('rudra_package_rudra_model','rudram');
	...
	**************/
    ...
    public function index()
    {
        ...
        $data['page_template'] = '_rudra_package';
        $data['page_header'] = ' Flea Package';
        $data['load_type'] = 'all';                
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'package_name', 'dnk_name', 'price', 'picture_count', 'package_description', 'dnk_description' ,'discount_price' );
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'rudra_package/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'rudra_package/post_actions';
				$action_url = 'rudra_package/post_actions';
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            if(!empty($data['form_data'])){
               $data['form_data']->package_description = implode(',', json_decode($data['form_data']->package_description));
                $data['form_data']->dnk_description = implode(',', json_decode($data['form_data']->dnk_description)); 
            }  
            //print_r($data);exit;

            $html = $this->load->view("crm/forms/_ajax_rudra_package_form", $data, TRUE);
        ...
        if($action == "update_data")
        {
            ...
            {
                $id = $_POST['id'];
                $package_description = json_encode((explode(",",$this->input->post('package_description',true))));
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_package').'_'.date('d-m-Y').".csv";
