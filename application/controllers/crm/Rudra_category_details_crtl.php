...
error_reporting(0);
class Rudra_category_details_crtl extends CI_Controller
{                   
    public function __construct()
    {
        parent::__construct();                
        $this->bdp = $this->db->dbprefix;
        $this->full_table = 'rudra_category_details';
        $this->return_data = array('status' => false, 'msg' => 'Error', 'login' => false, 'data' => array());
        //$this->set_data();
        $this->base_id = 0;
        $this->load->model('crudmaster_model','crd');
        $this->load->model('rudra_category_details_rudra_model','rudram');
	...
	**************/
    ...
    public function index()
    {
        ...
        $data['page_template'] = '_rudra_category_details';
        $data['page_header'] = ' Category Details';
        $data['load_type'] = 'all';                
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'category_name', 'dnk_name', 'status', 'is_deleted', );
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'rudra_category_details/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'rudra_category_details/post_actions';
				$action_url = 'rudra_category_details/post_actions';
				...
				$actions_button ="<a id='edt$row->id' href='javascript:void(0)' class='label label-success text-white f-12' onclick =\"static_form_modal('$form_data_url/get_data?id=$row->id','$action_url/update_data','md','Update Details')\" >Edit</a>";$checked = '';
                    $checked = '';
                if($row->status){
                    $checked = 'checked';
                }
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_category_details_form", $data, TRUE);
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_category_details').'_'.date('d-m-Y').".csv";
