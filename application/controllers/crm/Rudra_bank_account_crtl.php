...
error_reporting(0);
class Rudra_bank_account_crtl extends CI_Controller
{                   
    public function __construct()
    {
        ...
        $this->load->model('rudra_bank_account_rudra_model','rudram');
	...
	**************/
    ...
    public function index()
    {
        ...
        $data['page_template'] = '_rudra_bank_account';
        $data['page_header'] = ' Bank Account';
        $data['load_type'] = 'all';                
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'fk_user_id', 'bank_holder_name', 'bank_name', 'account_number', 'registration_number', );
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'rudra_bank_account/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'rudra_bank_account/post_actions';
				$action_url = 'rudra_bank_account/post_actions';
				...
				$actions_button ="<a id='edt$row->id' href='javascript:void(0)' class='label label-success text-white f-12' onclick =\"static_form_modal('$form_data_url/get_data?id=$row->id','$action_url/update_data','md','Bank Account Details')\" >Views</a>";
                $checked = '';
                if($row->status){
                    $checked = 'checked';
                }
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_bank_account_form", $data, TRUE);
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_bank_account').'_'.date('d-m-Y').".csv";
