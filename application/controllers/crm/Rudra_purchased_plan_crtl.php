...
error_reporting(0);
class Rudra_purchased_plan_crtl extends CI_Controller
{                   
    public function __construct()
    {
        ...
        $this->load->model('rudra_purchased_plan_rudra_model','rudram');
	...
	**************/
    ...
    public function index()
    {
        ...
        $data['pageTitle'] = ' Purchased Plan';                        
        $data['page_template'] = '_rudra_purchased_plan';
        $data['page_header'] = ' Purchased Package';
        $data['load_type'] = 'all';                
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'fk_plan_id', 'fk_user_id', 'fk_payment_method_id', 'transaction_id', 'purchased_key', 'purchased_date', 'expire_date', 'status', 'is_deleted', 'is_auto', );
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'rudra_purchased_plan/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'rudra_purchased_plan/post_actions';
				$action_url = 'rudra_purchased_plan/post_actions';
				...
				// $actions_button ="<a id='edt$row->id' href='javascript:void(0)' class='label label-success text-white f-12' onclick =\"static_form_modal('$form_data_url/get_data?id=$row->id','$action_url/update_data','md','Views Details')\" >Views</a>";$checked = '';
                $checked = '';
                if($row->status){
                    $checked = 'checked';
                }
				...
				$fk_plan_id = $this->db->get_where('rudra_plan',array('id'=>$row->fk_plan_id))->row();
				$row->fk_plan_id = (!empty($fk_plan_id) ? $fk_plan_id->plan_name:'--');
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_purchased_plan_form", $data, TRUE);
        ...
        if($action == "update_data")
        {
            if(!empty($_POST))
            {
				...
				array(
				 ...
				 'fk_plan_id' => $this->input->post('fk_plan_id',true),
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_purchased_plan').'_'.date('d-m-Y').".csv";
