...
error_reporting(0);
class Rudra_need_help_type_crtl extends CI_Controller
{                   
    public function __construct()
    {
        parent::__construct();                
        $this->bdp = $this->db->dbprefix;
        $this->full_table = 'rudra_need_help_type';
        $this->return_data = array('status' => false, 'msg' => 'Error', 'login' => false, 'data' => array());
        //$this->set_data();
        $this->base_id = 0;
        $this->load->model('crudmaster_model','crd');
        $this->load->model('rudra_need_help_type_rudra_model','rudram');
	...
	**************/
    ...
    public function index()
    {
        ...
        $data['pageTitle'] = ' Need Help Type';                        
        $data['page_template'] = '_rudra_need_help_type';
        $data['page_header'] = ' Need Help Type';
        $data['load_type'] = 'all';                
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'name', 'dnk_name', 'status', 'is_deleted', );
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'rudra_need_help_type/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'rudra_need_help_type/post_actions';
				$action_url = 'rudra_need_help_type/post_actions';
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_need_help_type_form", $data, TRUE);
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_need_help_type').'_'.date('d-m-Y').".csv";
