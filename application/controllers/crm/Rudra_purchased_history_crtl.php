...
error_reporting(0);
class Rudra_purchased_history_crtl extends CI_Controller
{                   
    public function __construct()
    {
        parent::__construct();                
        $this->bdp = $this->db->dbprefix;
        $this->full_table = 'rudra_purchased_history';
        $this->return_data = array('status' => false, 'msg' => 'Error', 'login' => false, 'data' => array());
        //$this->set_data();
        $this->base_id = 0;
        $this->load->model('crudmaster_model','crd');
        $this->load->model('rudra_purchased_history_rudra_model','rudram');
	...
	**************/
    ...
    public function index()
    {
        ...
        $data['pageTitle'] = ' Purchased History';                        
        $data['page_template'] = '_rudra_purchased_history';
        $data['page_header'] = ' Purchased History';
        $data['load_type'] = 'all';                
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'fk_user_id', 'fk_purchased_id', 'fk_type', );
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'rudra_purchased_history/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'rudra_purchased_history/post_actions';
				$action_url = 'rudra_purchased_history/post_actions';
				...
				$fk_purchased_id = $this->db->get_where('rudra_purchased_package',array('id'=>$row->fk_purchased_id))->row();
				$row->fk_purchased_id = (!empty($fk_purchased_id) ? $fk_purchased_id->id:'--');
				$data[] = $row;
			}
		}
		else
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_purchased_history_form", $data, TRUE);
        ...
        if($action == "update_data")
        {
            if(!empty($_POST))
            {
				...
				array(
				 ...
				 'fk_purchased_id' => $this->input->post('fk_purchased_id',true),
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_purchased_history').'_'.date('d-m-Y').".csv";
