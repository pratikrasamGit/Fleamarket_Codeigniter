...
error_reporting(0);
class Rudra_user_fav_markets_crtl extends CI_Controller
{                   
    public function __construct()
    {
        parent::__construct();                
        $this->bdp = $this->db->dbprefix;
        $this->full_table = 'rudra_user_fav_markets';
        $this->return_data = array('status' => false, 'msg' => 'Error', 'login' => false, 'data' => array());
        //$this->set_data();
        $this->base_id = 0;
        $this->load->model('crudmaster_model','crd');
        $this->load->model('rudra_user_fav_markets_rudra_model','rudram');
	...
	**************/
    ...
    public function index()
    {
        ...
        $data['pageTitle'] = ' User Fav Markets';                        
        $data['page_template'] = '_rudra_user_fav_markets';
        $data['page_header'] = ' User Fav Markets';
        $data['load_type'] = 'all';                
    ...
    public function list()
    {
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'rudra_user_fav_markets/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'rudra_user_fav_markets/post_actions';
				$action_url = 'rudra_user_fav_markets/post_actions';
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_user_fav_markets_form", $data, TRUE);
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_user_fav_markets').'_'.date('d-m-Y').".csv";
