...
error_reporting(0);
class Rudra_rent_space_purchased_crtl extends CI_Controller
{                   
    public function __construct()
    {
        ...
        $this->load->model('rudra_rent_space_purchased_rudra_model','rudram');
	...
	**************/
    ...
    public function index()
    {
        ...
        $data['pageTitle'] = ' Rent Space Purchased';                        
        $data['page_template'] = '_rudra_rent_space_purchased';
        $data['page_header'] = ' Rent Space Purchased';
        $data['load_type'] = 'all';                
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'fk_user_id', 'fk_rent_space_id', 'fk_payment_method_id', 'table_type', 'table_no', 'table_price', 'table_total_price', 'transaction_id', 'purchased_key', 'purchased_date', 'status', 'is_deleted', );
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'rudra_rent_space_purchased/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'rudra_rent_space_purchased/post_actions';
				$action_url = 'rudra_rent_space_purchased/post_actions';
				...
				$actions_button ="<a id='edt$row->id' href='javascript:void(0)' class='label label-success text-white f-12' onclick =\"static_form_modal('$form_data_url/get_data?id=$row->id','$action_url/update_data','md','View Details')\" >Views</a>";
                $checked = '';
                if($row->status){
                    $checked = 'checked';
                }
				...
				$fk_rent_space_id = $this->db->get_where('rudra_rent_space',array('id'=>$row->fk_rent_space_id))->row();
				$row->fk_rent_space_id = (!empty($fk_rent_space_id) ? $fk_rent_space_id->id:'--');
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_rent_space_purchased_form", $data, TRUE);
        ...
        if($action == "update_data")
        {
            if(!empty($_POST))
            {
				...
				array(
				 ...
				 'fk_rent_space_id' => $this->input->post('fk_rent_space_id',true),
				 ...
				 'table_price' => $this->input->post('table_price',true),
				 'table_total_price' => $this->input->post('table_total_price',true),
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_rent_space_purchased').'_'.date('d-m-Y').".csv";
