...
error_reporting(0);
class Rudra_settings_crtl extends CI_Controller
{                   
    public function __construct()
    {
        parent::__construct();                
        $this->bdp = $this->db->dbprefix;
        $this->full_table = 'rudra_settings';
        $this->return_data = array('status' => false, 'msg' => 'Error', 'login' => false, 'data' => array());
        //$this->set_data();
        $this->base_id = 0;
        $this->load->model('crudmaster_model','crd');
        $this->load->model('rudra_settings_rudra_model','rudram');
	...
	**************/
    ...
    public function index()
    {
        ...
        $data['page_template'] = '_rudra_settings';
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'st_key', 'st_meta', 'dnk_meta', 'status', 'is_deleted', );
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'rudra_settings/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'rudra_settings/post_actions';
				$action_url = 'rudra_settings/post_actions';
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_settings_form", $data, TRUE);
        ...
        if($action == "update_data")
        {
            if(!empty($_POST))
            {
				...
				array(
				 ...
				 'st_key' => $this->input->post('st_key',true),
				 'st_meta' => $this->input->post('st_meta',true),
				 'dnk_meta' => $this->input->post('dnk_meta',true),
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_settings').'_'.date('d-m-Y').".csv";
